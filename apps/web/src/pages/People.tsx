...
export default function People() {
  ...
  const buildWhatsAppUrl = (person: Person, owedToMe: number): string | null => {
  ...
  const openRemind = (person: Person, owedToMe: number) => {
    const waUrl = buildWhatsAppUrl(person, owedToMe);
    if (!waUrl) return;
    setRemindTarget({ person, owedToMe, waUrl });
  };
  ...
  const downloadQr = () => {
    if (!paymentSettings?.payment_qr) return;
    const link = document.createElement('a');
    link.href = paymentSettings.payment_qr;
  ...
  return (
    <Layout>
        ...
        <div className="bg-card border rounded-lg divide-y">
          {allPeopleDisplay.length === 0 ? (
            ...
            allPeopleDisplay.map(({ person, owed_to_me, i_owe, net }) => {
              const canRemind = owed_to_me > 0 && !!person.phone?.replace(/\D/g, '');
              return (
                <div key={person.id} className="w-full p-4 hover:bg-muted/50 transition-colors">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {canRemind && (
                        <button
                          onClick={() => openRemind(person, owed_to_me)}
      ...
      {/* Remind dialog */}
      <Dialog open={!!remindTarget} onOpenChange={(open) => !open && setRemindTarget(null)}>
          ...
          {remindTarget && (
            <div className="space-y-4">
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 w-full py-2 px-4 rounded-md bg-[#25D366] text-white text-sm font-medium hover:bg-[#22c05e] transition-colors"
              >
                <WhatsAppIcon />
                Open WhatsApp
              </a>
